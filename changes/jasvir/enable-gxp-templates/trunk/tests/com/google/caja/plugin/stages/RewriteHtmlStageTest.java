    ...
    for (Job job : new ArrayList<Job>(jobs.getJobs())) {
      ...
      node.acceptPreOrder(new Visitor() {
          public boolean visit(AncestorChain<?> ac) {
            Block extractedScript = ac.node.getAttributes()
            .get(RewriteHtmlStage.EXTRACTED_SCRIPT_BODY);
            if (extractedScript != null) {
              jobs.getJobs().add(new Job(new AncestorChain<Block>(extractedScript)));
            }
            DomTree extractedGxp = ac.node.getAttributes()
            .get(RewriteHtmlStage.EXTRACTED_GXP_BODY);
            if (extractedGxp != null) {
              jobs.getJobs().add(new Job(new AncestorChain<DomTree>(extractedGxp)));
            }
            return true;
          }
        }, null);
