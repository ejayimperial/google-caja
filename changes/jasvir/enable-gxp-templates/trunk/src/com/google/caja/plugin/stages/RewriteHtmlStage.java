...
public class RewriteHtmlStage implements Pipeline.Stage<Jobs> {
   ...
   * gxp script tag.
   */
  public static final SyntheticAttributeKey<DomTree> EXTRACTED_GXP_BODY
      = new SyntheticAttributeKey<DomTree>(DomTree.class, "extractedGxp");
  ...
  private void rewriteScriptTag(
    ...
    // The script contents.
    CharProducer scriptStream;
    if (src == null) {  // Parse the script tag body.
      scriptStream = textNodesToCharProducer(scriptTag.children(), true);
  ...
  private static boolean isGxpContentType(String contentType) {
    String mimeType = getMimeType(contentType);
    return ("text/gxp".equals(mimeType));
  }
