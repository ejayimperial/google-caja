...
public class QuasiBuilder {
  ...
  public static ParseTreeNode subst(
      String patternText, Map<String, 
      ParseTreeNode> bindings) {
    return getPatternNode(patternText).substitute(false, bindings);
  }
  ...
  public static ParseTreeNode subst(
      ...
      String patternText, Map<String, 
      ParseTreeNode> bindings) {
    return getPatternNode(patternText).substitute(synthetic, bindings);
  }
  ...
  public static ParseTreeNode substV(String patternText, Object... args) {
    ...
    ParseTreeNode result = subst(false, patternText, bindings);
    if (result == null) {
   ...
   * @see #subst(String, java.util.Map)
   */
  public static ParseTreeNode substSynthV(String patternText, Object... args) {
    if (args.length % 2 != 0) {
      throw new RuntimeException("Wrong # of args for subst()");
    }
    Map<String, ParseTreeNode> bindings = new HashMap<String, ParseTreeNode>();
    ...
    ParseTreeNode result = subst(true, patternText, bindings);
    if (result == null) {
      throw new NullPointerException(
          "'" + patternText + "' > " + bindings.keySet());
    }
