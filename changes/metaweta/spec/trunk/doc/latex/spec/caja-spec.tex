...
Cajita. \TODO{erights}{split this up into statement and expression context.} See section~\ref{subsec:cajita-reg} for further qualifying conditions. \\ } \hrule
...
The output of the Caja sanitizer is a JavaScript function called a \emph{module}.
The \emph{bound} variables of a module are those that appear as the names of
...
For example, one step of the cajoling process rewrites
\begin{alltt}
var list = new Array(6);
\end{alltt}
to
\begin{alltt}
var list = new (IMPORTS\_\_\_.Array)(6);
\end{alltt}
...
it can set, for instance, \code{IMPORTS\_\_\_.global = IMPORTS\_\_\_}.
...
\begin{description}
    ...
    \item[eval] The module's \code{IMPORTS\_\_\_} object has no ``\code{eval}'' function. 
    ...
    \item[Function] The JavaScript \code{Function} constructor is also absent.
...
of variable name conflicts. \TODO{erights}{Split up \ref{eq:caja-thunk} into equation and statement context rules.}
...
\begin{description}
  ...
  may be initialized---that is, Caja code may add or modify the properties of
  a constructor and its \code{prototype}---but are implicitly frozen on first 
  use. A constructor must not contain an explicit \code{return}.
