...
(function(global) {
   ...
   */
  function readPub(obj, name, POE, opt_shouldThrow) {
    name = String(name);
    var ext;
  ...
  function each(obj, fn, POE) {
    ...
    if (obj && obj.constructor.typeTag___ === 'Array') {
      ...
      for (var k in obj) {
        if (canEnumOwn(obj, k)) {
          if (fn(k, readPub(obj, k, POE)) === BREAK) {
            return;
          }
        }
      }
   ...
   */
  function callPub(obj, name, args, POE) {
    name = String(name);
    var ext;
  ...
  // The typeof operator returns 'undefined', 'boolean', 'number', 'object', or 'string'.
  // However, it returns 'object' for array literals and actual instances of
  // the constructors above.  Since we don't want to extend undefined, we have three
  // cases we can optimize.
  ...
  function getExtension(POE, obj, name) {
    console.log(POE.toSource());
  ...
  // Only uncajoled code should be able to call extend().
  // Cajoled calls to this are rewritten to point to ___.safeExtend().
