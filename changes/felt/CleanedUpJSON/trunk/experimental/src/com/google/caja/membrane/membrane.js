...
function makeMembrane(target) {
    ...
    case "function":
      return function () {
				if (caretaker.gate.enabled && checkCall(around,pname,arguments)) {
					...
					// don't want any resulting functions to point to the global scope
  ...
  function makeShadow(obj) {
    ...
    shadow.handleCall___ = function (p, args) {			
			...
			} else if (caretaker.gate.enabled && checkCall(obj, p, args)) {
        if (p === "name") { alert(checkCall(obj, p, args)); }
 ...
 * It's assumed that there is a variable named tame_%ClassName%___ in JSON format
...
function checkRead(obj, prop) {
  ...
  if (perms[prop] && perms[prop].read === true) {
    return true;
  } else if (perms[prop]) {
    caja.fail("Read of ",class,".",prop," fails: ",perms[prop].comment);
  } else {
...
function checkCall(obj, prop, args) {
  ...
  if (perms[prop] && perms[prop].call !== false) {
    if (perms[prop].write === true) {
      return true;
    } else {
      for (var i = 0; i < perms[prop].call.length; i++) {
        if (!args[i]) {
          caja.fail("Calling ",obj,".",prop," failed: Permissions mismatch"+
								" on arg ",i);
          return false;
        }
        if (perms[prop].call[i].arr) {
          var rhs = args[i];
          ...
          for (var k in perms[prop].call[i].arr) {
            if (rhs === perms[prop].call[i].arr[k]) {
              flipswitch = true;
            }
          }
          if (flipswitch !== true) {
            caja.fail("Calling ",class,".",prop," fails: arg ",
									args[i].toString()," is not a valid value.");
						return false;
          }
        } else if (perms[prop].call[i].filter) {
          var filterfunc = eval("("+perms[prop].call[i].filter+")");
          var testfunc = filterfunc(args[i]);
          if (testfunc === false) {
            caja.fail("Calling ",class,".",prop," fails: arg ",rhs.toString(),
                " is not a valid value.");
            return false;
          ...
          caja.fail("something wrong with API");
          return false;
        }
      }
    ...
    caja.fail("Setting of ",class,".",prop," is not callable: ",
          perms[prop].comment);
  } else {
...
function checkSet(obj, prop, rhs) {
  ...
  if (perms[prop] && perms[prop].write !== false) {
    if (perms[prop].write === true) {
      return true;
    } else {
      if (typeof rhs === "number") { rhs = rhs.toString(); }
      ...
      if (perms[prop].write.arr) {
        for (var i in perms[prop].write.arr) {
          if (rhs === perms[prop].write.arr[i]) { return true; }
        }
        caja.fail("Setting of ",class,".",prop," fails: RHS ",rhs.toString(),
          " is not a valid value.");
        return false;
      } else if (perms[prop].write.filter) {
        var filterfunc = eval("("+perms[prop].write.filter+")");
        var testfunc = filterfunc(rhs);
        if (testfunc === false) {
          caja.fail("Calling ",class,".",prop," fails: arg ",rhs.toString(),
              " is not a valid value.");
          return false;
