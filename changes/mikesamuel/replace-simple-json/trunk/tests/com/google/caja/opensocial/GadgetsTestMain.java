...
public class GadgetsTestMain {
  ...
  private void testGadget(URI gadget, JsonArray testResults,
    ...
    JsonArray messages = Json.newJsonArray();
  ...
  private int run(String[] argv) throws UriCallbackException {
    ...
    JsonArray testResults = Json.newJsonArray();
    JsonArray summary = Json.newJsonArray();
    Json.putJson(
        resultDoc,
        "buildInfo", BuildInfo.getInstance().getBuildInfo(),
        "timestamp", timestamp,
        "gadgets", testResults,
        "summary", summary);
