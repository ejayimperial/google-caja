...
public class CssPropertyPatterns {
  ...
  public static class Builder implements BuildCommand {
    public void build(List<File> inputs, List<File> deps, File output)
      ...
      CssSchema schema;
      CharProducer spsIn, fnsIn;
      try {
        spsIn = CharProducer.Factory.create(new InputStreamReader(
            new FileInputStream(symbolsAndPropertiesFile), "UTF-8"), sps);
        try {
          fnsIn = CharProducer.Factory.create(new InputStreamReader(
              new FileInputStream(functionsFile), "UTF-8"), fns);
          try {
