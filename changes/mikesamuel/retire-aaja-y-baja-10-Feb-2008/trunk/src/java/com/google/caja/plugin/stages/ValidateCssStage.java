...
public class ValidateCssStage implements Pipeline.Stage<Jobs> {
  ...
  public boolean apply(Jobs jobs) {
    ...
    for (Job job : jobs.getJobsByType(
             Job.JobType.CSS, Job.JobType.CSS_TEMPLATE)) {
      // The parsetree node is a CssTree.StyleSheet
      AncestorChain<CssTree> cssTree;
      if (job.getRoot().node instanceof CssTemplate) {
        AncestorChain<CssTemplate> tree = job.getRoot().cast(CssTemplate.class);
        cssTree = new AncestorChain<CssTree>(tree, tree.node.getCss());
      } else {
        cssTree = job.getRoot().cast(CssTree.class);
      }
