...
HtmlEmitter.prototype = {
  ...
  a: function (name, value) {
    ...
    if (name === 'value') {
      // Set the defaultValue for input elements, so that
      // HTMLFormElement.reset() properly resets the form.
      // We do this here instead of in the browser abstraction layer, because
      // this code should emulate a DOM being built for the first time, and
      // setting an attribute for a newly created node is different from
      // subsequent changes which should not update the default value.
      // See bug 855.
