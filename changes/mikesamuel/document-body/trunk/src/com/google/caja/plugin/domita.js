...
attachDocumentStub = (function () {
  ...
  function exportFields(object, fields) {
    for (var i = fields.length; --i >= 0;) {
      ...
      if (!count) {
        console.log('getterName=' + getterName + ', setterName=' + setterName);
  ...
  function attachDocumentStub(idSuffix, uriCallback, imports, pseudoBodyNode) {
    ...
    extend(TameBackedNode, TameNode);
    ...
    extend(TamePseudoNode, TameNode);
    ...
    extend(TameOpaqueNode, TameBackedNode);
    ...
    extend(TameTextNode, TameBackedNode);
    ...
    extend(TameCommentNode, TameBackedNode);
    ...
    extend(TameElement, TameBackedNode);
    ...
    function TameTableElement(node, editable) {
      ...
      exportFields(this, ['tBodies','tHead','tFoot']);
    }
    ...
    function TameHTMLDocument(doc, body, editable) {
      ...
      exportFields(this, ['documentElement', 'body']);
    }
    extend(TameHTMLDocument, TamePseudoNode);
    ...
    function TamePseudoElement(
        tagName, tameDoc, childNodesMaker, parentNodeMaker, innerHTMLMaker,
        editable) {
      TamePseudoNode.call(this, editable);
      this.tagName___ = tagName;
      this.tameDoc___ = tameDoc;
      this.childNodesMaker___ = childNodesMaker;
      this.parentNodeMaker___ = parentNodeMaker;
      this.innerHTMLMaker___ = innerHTMLMaker;
      exportFields(this, ['tagName', 'innerHTML']);
    }
    extend(TamePseudoElement, TamePseudoNode);
    ...
    TamePseudoElement.prototype.write = function (text) {
      // TODO(mikesamuel): Needs implementation
      cajita.log('Called document.write() with: ' + text);
    };
    ...
    function TameXMLHttpRequest() {
      exportFields(this, ['requestHeader', 'onreadystatechange', 'readyState',
                          ...
                          'status', 'statusText']);
<<<<<<< .mine
      console.log('Created new XHR: ' + this);
=======
      // TODO(ihab.awad): Implement
