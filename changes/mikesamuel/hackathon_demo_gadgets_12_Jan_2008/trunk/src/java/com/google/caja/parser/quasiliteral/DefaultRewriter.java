...
public class DefaultRewriter extends AbstractRewriter {
  ...
  public DefaultRewriter() {
      ...
      public ParseTreeNode fire(ParseTreeNode node, Scope scope) {
        ...
        if (match(node, bindings, "@x__")) {
          throw new RuntimeException(
              ...
              + ((Reference) node).getIdentifierName());
        }
        return NONE;
      }
    });
    ...
    addRule("set11", new Rule() {
      public ParseTreeNode fire(ParseTreeNode node, Scope scope) {
        ...
        if (match(node, bindings, "var @v") &&
            !scope.isFunction(getIdentifierName(bindings.get("v")))) {
          if (!scope.isGlobal(getIdentifierName(bindings.get("v")))) {
            return node;
          }
          return expandDef(
              new Reference((Identifier)bindings.get("v")),
    ...
    addRule("foreach1", new Rule() {
      public ParseTreeNode fire(ParseTreeNode node, Scope scope) {
        Map<String, ParseTreeNode> bindings
            = new LinkedHashMap<String, ParseTreeNode>();
        if (match(node, bindings, "for (var @k in @obj) @body")) {
          expandEntries(bindings, scope);
          return subst(
              bindings,
              "{(function () {"
              + "  var o___ = @obj;"
              + "  for (var @k in o___) {"
              + "    if (!___.canRead(o___, @k)) continue;"
              + "    @body;"
              + "  }"
              + "})();}");
        }
