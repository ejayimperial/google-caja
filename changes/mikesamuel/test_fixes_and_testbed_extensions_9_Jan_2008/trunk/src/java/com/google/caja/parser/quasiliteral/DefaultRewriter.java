...
public class DefaultRewriter extends AbstractRewriter {
  ...
  public DefaultRewriter() {
    ...
    addRule("synthetic0", new Rule() {
      public ParseTreeNode fire(ParseTreeNode node, Scope scope) {
        if (isSynthetic(node)) {
          return expandAll(node, scopeFor(node, scope));
        }
        return NONE;
      }
    });
    ...
    addRule("set11", new Rule() {
      public ParseTreeNode fire(ParseTreeNode node, Scope scope) {
        ...
        if (match(node, bindings, "var @v")) {
          String declaredName = getIdentifierName(bindings.get("v"));
          if (scope.isException(declaredName)) {
            // TODO(mikesamuel): A separate rule should ensure that untrusted
            // code does not see a raw exception.
            return node;
          }
          if (!scope.isFunction(declaredName)) {
            return expandDef(
                new Reference((Identifier)bindings.get("v")),
    ...
    // TODO(mikesamuel): Protect the raw exception from user code
    addRule("catch", new Rule() {
      public ParseTreeNode fire(ParseTreeNode node, Scope scope) {
        if (node instanceof CatchStmt) {
          return expandAll(node, new Scope(scope, node));
        }
        return NONE;
      }
    });
  ...
  private static Scope scopeFor(ParseTreeNode node, Scope parentScope) {
    if (node instanceof FunctionConstructor || node instanceof CatchStmt) {
      return new Scope(parentScope, node);
    } else {
      return parentScope;
    }
  }
