...
public class DefaultRewriterTest extends TestCase {
  ...
  private void checkFails(String input, String error) throws Exception {
    try {
      new DefaultRewriter().expand(parseText(input), mq);
  ...
  private void checkSucceeds(String input, String expectedResult) throws Exception {
    ...
    ParseTreeNode actualResultNode
        = new DefaultRewriter().expand(inputNode, mq);
    assertEquals(format(expectedResultNode), format(actualResultNode));
  }

  private void showTree(String resource) throws Exception {
    ParseTreeNode program = parseResource(resource);
    ParseTreeNode rewritten = new DefaultRewriter().expand(program, mq);
    System.err.println("program = " + format(program));
    System.err.println("rewritten = " + format(rewritten));
  }
