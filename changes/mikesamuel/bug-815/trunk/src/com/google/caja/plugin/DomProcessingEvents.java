...
final class DomProcessingEvents {
  ...
  static final class BeginElementEvent extends DomProcessingEvent {
    ...
    @Override boolean canOptimizeToInnerHtml(int depth) {
      return depth > 1 || !("option".equals(name) || "optgroup".equals(name)
                            || "tbody".equals(name) || "thead".equals(name)
                            || "tfoot".equals(name) || "tr".equals(name)
                            || "td".equals(name) || "th".equals(name)
                            || "param".equals(name));
