...
import com.google.caja.parser.TokenList;
...
public final class Parser extends ParserBase {
      ...
      switch (Keyword.fromString(t.text)) {
        ...
        case TRY:
        {
          ...
          if (sawFinally) {
            ...
            exvar.setComments(
                idNode.getAttributes().get(ParseTreeNode.COMMENTS));
            tq.expectToken(Punctuation.RPAREN);
  ...
  private void finish(ParseTreeNode n, Mark startMark) throws ParseException {
    FilePosition start;
  ...
  private void finish(
      ParseTreeNode n, FilePosition start, List<? extends Token<?>> comments)
      throws ParseException {
    if (!comments.isEmpty()) {
      n.getAttributes().set(ParseTreeNode.COMMENTS, new TokenList(comments));
    }

    FilePosition pos;
    if (tq.isEmpty() || tq.currentPosition() != start) {
      pos = FilePosition.span(start, tq.lastPosition());
    } else {
      ...
      pos = FilePosition.startOf(start);
    }
    n.getAttributes().set(ParseTreeNode.FILE_POSITION, pos);
  }
