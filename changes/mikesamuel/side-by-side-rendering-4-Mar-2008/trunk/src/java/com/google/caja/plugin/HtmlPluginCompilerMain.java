...
public final class HtmlPluginCompilerMain {
  ...
  private int run(String[] argv) {
    ...
    try {
      ...
      writeFile(outputJsFile, compiler.getJavascript(), mq);
      writeFile(outputCssFile, compiler.getCss(), mq);
    } finally {
      dumpMessages(mq, compiler.getMessageContext());
    }

    return compiler.getJobs().hasNoErrors() ? 0 : -1;
  }
  ...
  private void writeFile(
      File path, ParseTreeNode contents, final MessageQueue mq) {
    Writer w;
    try {
    ...
    Callback<IOException> exHandler = new Callback<IOException>() {
    ...
    TokenConsumer tc = contents.makeRenderer(w, exHandler);
    
    if (contents != null) {
      RenderContext rc = new RenderContext(new MessageContext(), true, tc);
      try {
        contents.render(rc);
        tc.noMoreTokens();
        w.write("\n");
  ...
  private static void dumpMessages(MessageQueue mq, MessageContext mc) {
    try {
