 ...
 *   {@code NaN} instead of the name {@code undefined}.  This is documented as
 *   a gotcha in the cajoler spec, and can be seen in the following code:<pre>
...
public final class ArrayIndexOptimization extends OptimizationPass {
  ...
  @Override
  protected void optBindings(
      Pair<String, String> optimization, Map<String, ParseTreeNode> bindings) {
    Expression key = (Expression) bindings.get("k");
    if (key != null) {
      // We are substituting into one of the array member access patterns above.
      // If it is not a numeric operator, then attach a + to the front so that
      // we know that the result is numeric even if our scope analysis is wrong
      // or the variable is undefined.
      if (!((key instanceof Operation
             && isNumberOperator(((Operation) key).getOperator()))
             || key instanceof NumberLiteral)) {
        bindings.put("k", Operation.create(Operator.IDENTITY, key));
      }
    }
  }
  ...
  static boolean isNumberOrUndefOperator(Operator o) {
    ...
    return o == Operator.VOID || isNumberOperator(o);
  }

  static boolean isNumberOperator(Operator o) {
    if (o.getAssignmentDelegate() != null) {
      o = o.getAssignmentDelegate();
    }
