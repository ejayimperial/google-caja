...
package com.google.caja.quasiliteral;
...
public class MatchTest extends TestCase {
  ...
  public void testFunctionIdentifier() throws Exception {
    List<QuasiNode.QuasiMatch> m = match(
        ...
        "function x() { }");
    assertEquals(1, m.size());
    assertEquals(FunctionDeclaration.class, m.get(0).getRoot().getClass());
  ...
  public static List<QuasiNode.QuasiMatch> match(String pattern, String source) throws Exception {
    QuasiNode qn = QuasiBuilder.parseQuasiNode(pattern);
  ...
  public static ParseTreeNode parse(String src) throws Exception {
    InputSource inputSource = new InputSource(new URI("file:///artificial/input/source.js"));
    Parser parser = new Parser(
        new JsTokenQueue(
            new JsLexer(
                CharProducer.Factory.create(new StringReader(src),
                inputSource)),
            inputSource,
