...
public final class HtmlDefinitions {
  ...
  public static Block generateJavascriptDefinitions(HtmlSchema schema) {
    ...
    }
    {
      ...
      for (Map.Entry<String, HTML.Attribute.Type> e : atypes.entrySet()) {
        String key = e.getKey();
        String elementName = key.substring(0, key.indexOf(':'));
        if ("*".equals(elementName) || schema.isElementAllowed(elementName)) {
  ...
  private static Map<String, HTML.Attribute.Type> attributeTypes(
    ...
    for (String elAndAttrib : schema.getAttributeNames()) {
      int colon = elAndAttrib.indexOf(':');
      String elementName = elAndAttrib.substring(0, colon),
          attributeName = elAndAttrib.substring(colon + 1);
      if (schema.isAttributeAllowed(elementName, attributeName)) {
        ...
        attributeFlags.put(elAndAttrib, type);
      }
    }
