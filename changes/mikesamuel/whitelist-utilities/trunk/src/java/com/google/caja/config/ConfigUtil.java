...
public class ConfigUtil {
  ...
  public static WhiteList loadWhiteListFromJson(
    ...
    return wll.loadFrom(wll.expectJSONObject(JSONValue.parse(in), "whitelist"));
  }
...
class JSONWhiteListLoader {
  ...
  WhiteListSkeleton makeSkeleton(
    ...
    // * Abort if there exist any two distinct type definitions in W with the
    //   same key.
    for (List<JSONObject> defs : w.definitions.values()) {
      ...
      for (JSONObject other : rest) {
        if (!other.equals(definition)) {
          mq.addMessage(ConfigMessageType.AMBIGUOUS_DEFINITION, src,
                        MessagePart.Factory.valueOf("" + definition),
                        MessagePart.Factory.valueOf("" + other));
          break;
        }
      }
      rest.clear();
