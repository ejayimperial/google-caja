...
class JsdocRewriter {
    ...
    if (fileOverview != null) {
      return QuasiBuilder.substV(
          ...
          "fileName", StringLiteral.valueOf(
              format(js.getFilePosition().source())),
  ...
  ParseTreeNode rewritePackageDocs(InputSource is, Comment cmt) {
    ...
    return QuasiBuilder.substV(
        ...
        "packageName", StringLiteral.valueOf(format(is)),
  ...
  private ParseTreeNode rewrite(ParseTreeNode js) {
    ...
    if (js instanceof FunctionConstructor
      ...
      result = ParseTreeNodes.newNodeInstance(
          js.getClass(), js.getValue(), rewriteAll(js.children()));
    }
    ((AbstractParseTreeNode<?>) result).setFilePosition(js.getFilePosition());
    return result;
  }
  ...
  private Expression documentExpression(Expression js, Expression doc) {
        ...
        switch (op) {
          ...
          case STRICTLY_NOT_EQUAL:
            ...
            return Operation.create(
                op, unwrappedChildren.toArray(new Expression[0]));
          case LOGICAL_OR:
            return (Expression) QuasiBuilder.substV(
    ...
    if (rewritten instanceof Reference
      ...
      return new Reference(new Identifier("_iterator_"));
    }
  ...
  private ExpressionStmt documentExpressionStmt(ExpressionStmt js) {
    ...
    return new ExpressionStmt(documentExpression(js.getExpression(), doc));
  }
  ...
  private ParseTreeNode documentFunction(ParseTreeNode fn) {
    ...
    for (Map.Entry<String, Expression> e : fieldDocsByName.entrySet()) {
      ...
      if (memberDoc == null) { memberDoc = new ObjectConstructor(); }
      fieldMembers.add(Pair.pair(
          (Literal) StringLiteral.valueOf(e.getKey()), memberDoc));
    }
    ObjectConstructor fields = new ObjectConstructor(fieldMembers);
    ...
    if (decl == null) {
      ...
      FunctionDeclaration newDecl = new FunctionDeclaration(
          newCtor.getIdentifier(), newCtor);
      Statement docCall = new ExpressionStmt((Expression) QuasiBuilder.substV(
  ...
  private Loop documentLoop(Loop js) {
    ...
    return ParseTreeNodes.newNodeInstance(
        js.getClass(), js.getLabel(), rewritten);
  }
  ...
  private Conditional documentConditional(Conditional js) {
    ...
    return ParseTreeNodes.newNodeInstance(
        js.getClass(), js.getValue(), rewritten);
  }
  ...
  private Statement delayDocingOfUninitializedVariables(final Statement s) {
    ...
    for (Declaration decl : uninitialized) {
      ...
      if (doc != null) {
        docStmts.add(new ExpressionStmt((Expression) QuasiBuilder.substV(
    ...
    Statement rewrittenStmt = ParseTreeNodes.newNodeInstance(
        s.getClass(), s.getValue(), rewriteAll(s.children()));
    if (docStmts.isEmpty()) {
  ...
  private Expression getDocCommentJson(ParseTreeNode js) {
    ...
    Expression json = commentToJson(cmt);
    if (json != null) {
      ((AbstractParseTreeNode<?>) json).setFilePosition(js.getFilePosition());
    }
    return json;
  }
    ...
    if (!description.isEmpty()) {
      ...
      if (descExpr != null) {
        docEntries.add(Pair.pair(
            (Literal) StringLiteral.valueOf("@description"), descExpr));
      }
      if (summaryExpr != null) {
        docEntries.add(Pair.pair(
            (Literal) StringLiteral.valueOf("@summary"), summaryExpr));
      }
    }

    docEntries.add(Pair.pair(
        (Literal) StringLiteral.valueOf("@pos"),
        (Expression) StringLiteral.valueOf(format(cmt.getFilePosition()))));
    for (Map.Entry<String, List<Expression>> block : blocks.entrySet()) {
      docEntries.add(Pair.pair(
          (Literal) StringLiteral.valueOf("@" + block.getKey()),
          (Expression) new ArrayConstructor(block.getValue())));
    }
    return new ObjectConstructor(docEntries);
  }

  private String format(MessagePart p) {
