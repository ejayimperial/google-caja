 ...
 * com.google.caja.tools.jsdoc.Jsdoc.
 * <p>
 * This assumes that the HTML will run in an environment where:
 * <ul>
 * <li>{@code getJsdocRoot()} returns the JSON documentation tree</li>
 * <li>{@code navigateToApiElement(apiElementName)}
 *   makes documentation for the named API element visible, and returns
 *   {@code undefined}.</li>
 * </ul>
 * <li>{@code navigateToFile(path)} shows the documentation for a particular
 *   input source file.  The file path may be a directory</li>
 * <li>The styles from <tt>jsdoc.css</tt> are loaded.
 * </ul>
 *
...
function lookupFile(path) {
  path = path.replace(/^[/\\]+|[/\\]+$/g, '');
  var jsonDoc = getJsonDocRoot()['@fileoverview'];
  if (path) {
...
(function () {
  ...
  renderCompactIndex = function (jsonDocRoot, htmlOut) {
    ...
    renderCompactIndexHelper(
        jsonDocRoot,
        function (name) {
          return 'navigateToApiElement(' + jsstring(name) + ')';
        }, 'compactindex:api', 'globals', '.', [], htmlOut);
    renderCompactIndexHelper(
        jsonDocRoot['@fileoverview'],
        function (name) {
          return 'navigateToFile(' + jsstring(name) + ')';
        }, 'compactindex:file', 'files', '/', [], htmlOut);
    htmlOut.push('</ul>');
  };
  ...
  var transforms = {
        ...
        '@extends': function (v) {
          ...
          if (lookupApiElement(v)) {
            return '<span onclick="navigateToApiElement('
                + html(jsString(v)) + ')" class="type link">' + html(v)
                + '</span>';
          }
        ...
        for (var i = 0, n = members.length; i < n; ++i) {
          ...
          if (member.navigationLink) {
            htmlOut.push(
                ...
                '" href="#" onclick="', html(member.navigationLink),
                ';return false">');
          }
    ...
    renderChunk(
        ...
        function (apiElementName, flags, jsonDoc, htmlOut) {
          ...
          htmlOut.push(
              ...
              (jsonDoc['@pos']
               ? ' @ <span class="pos">' + jsonDoc['@pos'] + '</span>'
               : ''),
              '</div>');
        },
          ...
          if (!apiElementName) {
            ...
            sections.push({ name: apiElementName,
                          ...
                          { name: apiElementName + '.@field',
                            ...
                            // navigateToApiElement function won't work.
                            navigable: false
                          });
          } else {
            sections.push({ name: apiElementName,
          ...
          var subfiles = map(
              ...
              function (fileName) {
                ...
                return {
                    ...
                    qualifiedName: qualifiedName,
                    navigationLink:
                        'navigateToFile(' + jsstring(qualifiedName) + ')'
                  };
              });
          var apiElements = map(
              ...
              function (apiElementName) {
                return {
                    ...
                    navigationLink:
                        'navigateToApiElement(' + jsstring(apiElementName) + ')'
                  };
              });
  ...
  function renderCompactIndexHelper(
      jsonDoc, apiLink, idPrefix, name, separator, nameParts, htmlOut) {
    ...
    htmlOut.push(
        '&nbsp;<a onclick="', html(apiLink(apiElementName)), ';return false"',
        ' href="#" title="', titleHtml(summaryHtml), '">',
        html(name), '</a>');
    if (expandable) {
      ...
      for (var i = 0, n = childNames.length; i < n; ++i) {
        ...
        nameParts[depth] = k;
        renderCompactIndexHelper(
            jsonDoc[k], apiLink, idPrefix, k, separator, nameParts, htmlOut);
      }
