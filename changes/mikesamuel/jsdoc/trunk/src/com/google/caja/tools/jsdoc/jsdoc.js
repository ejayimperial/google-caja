...
var jsdoc___ = (function () {
  ...
  function updoc(runs) {
    var htmlBuf = [];
    htmlBuf.push('<pre class="prettyprint lang-js">');
    for (var i = 0, n = runs.length; i < n; ++i) {
      ...
      try {
        ...
        if (structurallyEquivalent(actual, expected)) {
          ...
          status = 'fail';
          try {
            reason = '  // Was ' + actual + ' : ' + typeNameOf(actual);
          } catch (ex) {
            reason = '  // Mismatch';
          }
        }
  ...
  function runtimeMessage(msg, level) {
    jsdocMessageQueue__.addMessage(
        Packages.com.google.caja.tools.jsdoc.JsdocMessageType.RUNTIME_MESSAGE,
        Packages.com.google.caja.reporting.MessageLevel[level],
        Packages.com.google.caja.reporting.MessagePart.Factory.valueOf(
            '' + msg));
  }
  ...
  function walkObjectGraph() {
    ...
    function visitDocs(obj, docs, nameParts) {
      ...
      if (nameParts.join('.') === 'a') {
        console.log('obj=' + obj + ', type=' + type + ', underlying=' + underlying);
        }
  ...
  function linkTo(apiElement, apiElementName, linkTarget, givenName) {
    ...
    if (targetName) {
      ...
      error('Cannot resolve ' + givenName + ' in context of ' + apiElementName);
      return '#';
    }
  }
