...
import com.google.caja.tools.jsdoc.Updoc.Run;
...
final class UpdocParser {
  ...
  Updoc parse(JsTokenQueue tq) throws ParseException {
    ...
    do {
      ...
      tq.checkToken(";");
      Updoc.Run run = new Updoc.Run(input, result);
      run.setFilePosition(FilePosition.span(start, tq.lastPosition()));
      runs.add(run);
    } while (tq.lookaheadToken("$"));
    Updoc updoc = new Updoc(runs);
    updoc.setFilePosition(
        FilePosition.span(
