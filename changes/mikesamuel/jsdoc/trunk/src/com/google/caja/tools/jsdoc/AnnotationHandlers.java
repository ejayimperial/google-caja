...
public final class AnnotationHandlers {
  ...
  private static class TextAnnotationHandler implements AnnotationHandler {
    public Expression handle(Annotation a, MessageQueue mq) {
      StringLiteral sl = StringLiteral.valueOf(a.getValue());
      sl.setFilePosition(a.getFilePosition());
      return sl;
    }
  }
   ...
   */
  private static final Expression OK = new NullLiteral();
      ...
      public Expression handle(Annotation a, MessageQueue mq) {
        ...
        if (content.startsWith("<")) {
          ...
          try {
            ...
            DomTree.Fragment f = parser.parseFragment();
            parser.getTokenQueue().expectEmpty();
            if (f.children().isEmpty()
                || !(f.children().get(0) instanceof DomTree.Tag)
                || !"a".equals(f.children().get(0).getValue())) {
              link = null;
            } else {
              link = (DomTree.Tag) f.children().get(0);
            }
          ...
          if (link != null) {
            ...
            if (href != null) {
              URI uri = parseDocUri(
                  ...
                  href.getAttribValueNode().getFilePosition(), mq);
              if (uri != null) {
                StringBuilder name = new StringBuilder();
                TokenConsumer tc = link.makeRenderer(name, null);
                RenderContext rc = new RenderContext(mc, tc);
                for (DomTree child : link.children()) {
                ...
                return (Expression) QuasiBuilder.substV(
                    "({ 'name': @name, 'url': @url })",
                    "name", StringLiteral.valueOf(name.toString()),
                    "url", StringLiteral.valueOf(uri.toString()));
              }
            }
          }
        }
  ...
  private AnnotationHandler identifier() {
    return new AnnotationHandler() {
      public Expression handle(Annotation a, MessageQueue mq) {
        ...
        return StringLiteral.valueOf(value);
      }
    };
  }
  ...
  private AnnotationHandler list(final AnnotationHandler... itemHandlers) {
    ...
    return new AnnotationHandler() {
      public Expression handle(Annotation a, MessageQueue mq) {
        ...
        if (item == null) { return null; }
        items.add(item);
        return new ArrayConstructor(items);
      }
    };
  }
      ...
      public Expression handle(Annotation a, MessageQueue mq) {
        ...
        try {
          Updoc updoc = new UpdocParser(mq).parseComplete(
              CharProducer.Factory.create(new StringReader(value),
              a.getFilePosition()));
          ...
          for (Updoc.Run run : updoc.getRuns()) {
            runs.add((ObjectConstructor) QuasiBuilder.substV(
                ...
                + "   pos:    @runPos })",
                "runSource", StringLiteral.valueOf(render(run, false)),
                "input", run.getInput(),
                "result", run.getResult(),
                "runPos", StringLiteral.valueOf(format(run.getFilePosition()))
                ));
          }
    ...
    return new AnnotationHandler() {
      public Expression handle(Annotation a, MessageQueue mq) {
        ...
        for (Pair<String, AnnotationHandler> entry : tokenEntries) {
          ...
          if (value == null) { return null; }
          mapEntries.add(
              Pair.pair((Literal) StringLiteral.valueOf(entry.a), value));
        ...
        if (value == null) { return null; }
        mapEntries.add(
            Pair.pair((Literal) StringLiteral.valueOf(remainder.a), value));
        return new ObjectConstructor(mapEntries);
      }
    };
  }
  ...
  private AnnotationHandler tee(final AnnotationHandler... handlers) {
    return new AnnotationHandler() {
      public Expression handle(Annotation a, MessageQueue mq) {
        ...
        return new ArrayConstructor(results);
      }
    };
  }
        ...
        if (s != null) {
          ...
          if (s.startsWith("{") && s.endsWith("}")) {
            ...
            while (m.find()) {
              ...
              symbolNames.add(StringLiteral.valueOf(ident));
            }
            StringLiteral type = StringLiteral.valueOf(typeString);
            if (symbolNames.isEmpty()) { return type; }
            return (Expression) QuasiBuilder.substV(
                "jsdoc___.requireTypeAtoms(@pos, [@symbols*]), @type",
                "pos", StringLiteral.valueOf(format(a.getFilePosition())),
          ...
          if (name != null || uri != null) {
            ...
            if (name != null) {
              entries.add(Pair.<Literal, Expression>pair(
                  StringLiteral.valueOf("name"), StringLiteral.valueOf(name)));
            }
            if (uri != null) {
              entries.add(Pair.<Literal, Expression>pair(
                  StringLiteral.valueOf("url"),
                  StringLiteral.valueOf("" + uri)));
            }
            return new ObjectConstructor(entries);
          }
        }
        mq.addMessage(
            JsdocMessageType.EXPECTED_EMAIL_OR_NAME,
  ...
  private Expression toIdentifierChain(
    ...
    for (String part : parts) {
      if (!IDENTIFIER.matcher(part).matches() || Keyword.isKeyword(part)) {
        mq.addMessage(
            JsdocMessageType.EXPECTED_IDENTIFIER, pos,
            MessagePart.Factory.valueOf("'" + s + "'"));
        return null;
      }
      Reference r = new Reference(new Identifier(part));
      e = e == null ? r : Operation.create(Operator.MEMBER_ACCESS, e, r);
    }
    return e;
  }
  ...
  private static Expression concatenate(Expression a, Expression b) {
    if (a == null) { return b; }
    return Operation.create(Operator.ADDITION, a, b);
  }
