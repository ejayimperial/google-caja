...
public class LookaheadCharProducer implements CharProducer {
  ...
  public LookaheadCharProducer(CharProducer p, int lookaheadLength) {
  ...
  public final FilePosition getCurrentPosition() {
    FilePosition fp;
  ...
  public final boolean getCurrentPosition(MutableFilePosition posBuf) {
    boolean result;
    if (lookaheadLimit == lookaheadPos) {
   ...
   */
  public final int lookahead() throws IOException {
    fetch(1);
    return lookahead[lookaheadPos];
  }

  /** Consume n characters from the lookahead buffer. */
  final void consume(int nChars) {
   ...
   * lookahead buffer.
   */
  public final void fetch(int n) throws IOException {
    int limit;
    if (lookaheadLimit == lookaheadPos) {
      lookaheadLimit = lookaheadPos = 0;
      limit = n;
