...
public class GadgetRewriterMain {
  ...
  GadgetRewriteException, IOException, UrlCallbackException {
    System.exit(new GadgetRewriterMain().run(argv));
  }

  class Callback implements UrlCallback {
    ...
    @Override
    public InputStream retrieve(final URL url, final String mimeType)
        throws UrlCallbackException {
    ...
    @Override
    public URL rewrite(URL url, String mimeType) {
  ...
  private int run(String[] argv) throws UrlCallbackException,
  ...
  private void runOnce() throws IOException, UrlCallbackException,
      GadgetRewriteException, MalformedURLException {
    GadgetRewriter rewriter = new DefaultGadgetRewriter();
    Writer w = new BufferedWriter(new FileWriter(outputFile));
    rewriter.rewrite(new URL(gadgetUrl), new Callback(), w);
    w.flush();
    w.close();
  }
