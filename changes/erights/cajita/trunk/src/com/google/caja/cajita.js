...
if (Function.prototype.bind === (void 0)) {
   ...
   * special knowledge of Cajita. This allows Cajita code using bind() to
...
(function(global) {
   ...
   * B (i.e., if A's prototype is B) and B is frozen, A may or may not be
   * frozen.
   * <p>
   * If typeof <tt>obj</tt> is neither 'object' nor 'function', then
   * it's considered frozen.
   */
...
//    fastpathSet(obj, name);
  ...
  /**
   * Can a Cajita client of <tt>obj</tt> read its <name> property?
   * <p>
  ...
  /**
   * Cajita code attempting to read a property on something besides
   * <tt>this</tt>.
   * <p>
   ...
   * <p>
   * "Innocent" code is code which we assume to be ignorant of Cajita,
   ...
   * to co-exist smoothly with the Cajita runtime.
   * <p>
   ...
   * Cajita code. However, it is not up to Cajita to shield innocent code
  ...
  /**
   * Inside a <tt>cajita.each()</tt>, the body function can terminate
   * early, as if with a conventional <tt>break;</tt>, by doing a
   * <pre>return cajita.BREAK;</pre>
   */
   ...
   * If this property is not Internal and was defined by Cajita code,
   ...
   * mutation from violating Cajita semantics. In order for this fault
   ...
   * the logic from cajita-debugmode.js to here.
   */
  ...
  each(cajita, simpleFrozenFunc(function(k, v) {
    switch (typeof v) {
      case 'object': {
