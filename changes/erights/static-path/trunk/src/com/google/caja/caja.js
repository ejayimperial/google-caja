...
//    fastpathSet(obj, name);
  ...
  function derive(constr, opt_Sup) {
    if (opt_Sup) {
      ...
      if (!isFrozen(constr.prototype)) {
        // So that tamed prototypes whose constructors are
        // non-deletable can still be walked with 
        // directConstructor().
  ...
  function attach(self, meth) {
    ...
    if (same(meth.___ATTACHMENT___, self)) {
      return meth;
    }
  ...
  var Attribute = new RegExp("(.*[^_])_[canRead|canCall|getter|handler]*___$");
