...
(function(global) {
  ...
  function object(obj) {
    if (!isJSONContainer(obj)) {
      fail('caja.object(obj) applies only to JSON Containers: ',
           debugReference(obj));
    }
    var keys = [];
    for (key in obj) {
      if (canEnumOwn(obj, key)) {
        keys.push(key);
      }
    }
    for (var i = 0; i < keys.length; i++) {
      ...
      if (typeof v === 'function') {
        if (!isCtor(v) && !isMethod(v) && !isXo4aFunc(v) && !isSimpleFunc(v)) {
          v = simpleFunc(v, k);
        }
        v = primFreeze(v);
      }
...
//    fastpathSet(obj, name);
      ...
      switch (typeof ex) {
        case 'object': {
          ...
          if (isInstanceOf(ex, Error)) {
            ...
            return object({ message: message, name: name, stack: stack });
          }
          return '' + ex;
        }
        case 'string':
   ...
   */
  var ignoreNewModule = object({
    handle: function(newModule){}
  });
  ...
  function makeSealerUnsealerPair() {
    ...
    return object({ seal: seal, unseal: unseal });
  }
