...
import com.google.caja.render.TabularSideBySideRenderer;
...
public final class PluginCompilerMain {
  ...
  private final Config config = new Config(
      getClass(), System.err,
      "Cajoles HTML, CSS, and JS files to JS & CSS.");
  ...
  private void writeFile(File f, ParseTreeNode output) {
    ...
    try {
      ...
      try {
        TokenConsumer tc = config.sideBySideOutput() ?
            new TabularSideBySideRenderer(
                buildOriginalInputCharSequences(), mc, out, ioHandler) :
            output.makeRenderer(out, ioHandler);
