...
jsunitRegister('testRemoveEventListener',
  ...
  var failed = false;
  function second(node, event) {
    console.log('received event');
    if (failed) { return; }
    assertEquals('P', node.tagName);
    assertEquals('click', event.type);
    node.innerHTML = '<b>All done!</b>';
    pass('test-remove-event-listener');
  }
  function first(node, event) {
    if (firstFired) {
      node.innerHTML = '<b>FAILED - event handler was not removed!</b>';
      failed = true;
      return;
    }
    firstFired = true;
    ...
    assertEquals('P', node.tagName);
    assertEquals('click', event.type);
    node.innerHTML = '<b>Thank you, click me again please</b>';
...
//  var container = document.getElementById('test-case-insensitive-attrs');
  ...
  tableNode.addEventListener('click', function(node, event) {
