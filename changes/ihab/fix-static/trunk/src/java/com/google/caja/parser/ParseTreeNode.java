...
public interface ParseTreeNode extends MessagePart, Renderable, Cloneable {
   ...
   *
   * This is used to ensure that all nodes have been "seen" by a security
   * checking or other critical process. The input is first marked "tainted",
   * and the code is written such that any concrete step that recognizes a node
   * un-taints it (or produces an un-tainted result as the output). The process
   * can then check to make sure that all its output is not tainted.
   */
