...
public final class TestUtil {
  ...
  public static void checkFilePositionInvariants(ParseTreeNode n) {
    ...
    try {
      ...
      msg = (next == null) ?
          msg + " - next = null" :
          msg + " - next = " + next + " : " + next.getFilePosition();

      TestCase.assertTrue(msg, null == next
      ...
      if (!children.isEmpty()) {
        ...
        TestCase.assertTrue(msg, first.getFilePosition().startCharInFile()
                                 >= n.getFilePosition().startCharInFile());
        TestCase.assertTrue(msg, last.getFilePosition().endCharInFile()
                                 <= n.getFilePosition().endCharInFile());
      }

      for (ParseTreeNode c : children) {
        checkFilePositionInvariants(c);
      }
