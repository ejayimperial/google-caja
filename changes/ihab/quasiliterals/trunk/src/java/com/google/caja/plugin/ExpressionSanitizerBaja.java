...
public class ExpressionSanitizerBaja {
  ...
  private final class FieldAccessRewriter implements Visitor {
    ...
    private Operation anyMember(String instance, String member) {
      return new Operation(Operator.MEMBER_ACCESS, new Reference(new Identifier(instance)),
    ...
    private Operation call___(String what, Expression... args) {
      ...
      callChildren[0] = new Operation(Operator.MEMBER_ACCESS,
          new Reference(new Identifier("___")), new Reference(new Identifier(what)));
    ...
    private Operation privateScope(Block body) {
      final FunctionConstructor tempFn = new FunctionConstructor(new Identifier(null),
    ...
    private Operation simpleCanTernary(boolean isThis, String subject,
      ...
      callChildren[0] = new Operation(
          ...
          new Reference(new Identifier(canWhat.toLowerCase() + canType)));
      callChildren[1] = new Reference(new Identifier(subject));
    ...
    private Operation canTernary(Expression lhs, String canWhat, String rhsName,
      ...
      final Operation ternary = simpleCanTernary(isThis, isRef
          ? ((Reference)lhs).getIdentifierName() : ReservedNames.TEMP, canWhat, rhsName, op,
          args);
      if (isRef) {
        return ternary;
      }
      ...
      if (node instanceof FunctionConstructor) {
        ...
        if (finder.thisUsed) {
          final Statement localThis = new Declaration(
              ...
              new Reference(new Identifier("this")));
        ...
        if (finder.argumentsUsed) {
          final Statement localArgs
            = new Declaration(new Identifier(ReservedNames.LOCAL_ARGUMENTS),
                call___("args", new Reference(new Identifier("arguments"))));
        ...
        final Reference ref = (Reference) node;
        if (ref.isThis()) {
          ref.setIdentifierName(ReservedNames.LOCAL_THIS);
        } else if (ref.isArguments()) {
          ref.setIdentifierName(ReservedNames.LOCAL_ARGUMENTS);
        }
        ...
        final Operation call = new Operation(Operator.FUNCTION_CALL,
            new Reference(new Identifier("RegExp")), string(regex.getMatchText()),
        ...
        if (operator == Operator.MEMBER_ACCESS) {
          final String rhsName = ((Reference)rhs).getIdentifierName();
