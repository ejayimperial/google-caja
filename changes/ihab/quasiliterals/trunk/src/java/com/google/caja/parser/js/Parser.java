...
public final class Parser extends ParserBase {
  ...
  private LabeledStatement parseLoopOrSwitch(String label)
    ...
    switch (Keyword.fromString(t.text)) {
      ...
      case DO:
      {
        ...
        Expression cond = parseExpressionInt(true);
        tq.expectToken(Punctuation.RPAREN);
        s = new DoWhileLoop(label, cond, body);
        break;
      }
      case SWITCH:
      {
        ...
        case FUNCTION:
        {
          ...
          if (tq.lookaheadToken(Punctuation.LPAREN)) {
            ...
            setFilePositionToStart(
                fcIdNode,
                params.params.isEmpty() ? (AbstractParseTreeNode)body : params.params.get(0));
    ...
    if (isDeclaration) {
      ...
      Declaration d;
      {
        ...
        if (initializer == null) {
          ...
          setFilePositionToStart(idNode, (AbstractParseTreeNode)initializer);
        }
      }
  ...
  private void setFilePositionToStart(AbstractParseTreeNode n, AbstractParseTreeNode next) {
    n.setFilePosition(FilePosition.startOf(next.getFilePosition()));
  }

  private void setFilePositionToEnd(AbstractParseTreeNode n, AbstractParseTreeNode next) {
