 ...
 */
public class TrailingUnderscoresMatchNode extends QuasiMatchNode {
  private final String trailing;

  public TrailingUnderscoresMatchNode(String identifier, int numberOfUnderscores) {
    ...
    if (specimens.size() > 0 && isCompatibleClass(specimens.get(0))) {
      ...
      if (value.endsWith(trailing)) {
        ...
        return putIfDeepEquals(
            bindings,
            getIdentifier(),
            AbstractParseTreeNode.newNodeInstance(
                Identifier.class,
                value.substring(
                    0,
