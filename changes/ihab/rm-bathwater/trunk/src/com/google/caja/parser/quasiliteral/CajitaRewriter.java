...
public class CajitaRewriter extends Rewriter {
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // TODO(ihab.awad): Add these before checking in
  // - Disallow all 'prototype' refs & test
  // - Disallow all 'constructor' refs & test
  // - Change 'simpleFunc' to 'constructFunc' or whatever
  // - Test the use of 'caja.construct()' on both "non-call" and "call" syntaxes
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  final public Rule[] cajaRules = {
    ...
    new Rule() {
      ...
      @RuleDescription(
          ...
          reason="Builds the module body encapsluation around the Cajita "
              + "code block.",
          matches="{@ss*;}",
    ...
    new Rule() {
      ...
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
        ...
        if (bindings != null) {
          mq.addMessage(
              RewriterMessageType.FOREACH_NOT_IN_CAJITA,
    ...
    new Rule() {
      ...
      @RuleDescription(
          name="varThisBad",
          synopsis="The \"this\" variable is not permitted in Cajita.",
    ...
    new Rule() {
      @Override
      @RuleDescription(
          name="varDeclaredFunc",
          synopsis="",
          reason="",
          matches="@func",
          substitutes="___.primFreeze(@func)")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
        Map<String, ParseTreeNode> bindings = match(node);
        if (bindings != null
            && scope.isDeclaredFunctionReference(bindings.get("func"))) {
          return subst(bindings);
        }
        return NONE;
      }
    ...
    new Rule() {
      ...
      @RuleDescription(
          name="setBadThis",
          synopsis="Statically reject assignment to 'this'.",
          reason="Cajita bans any reference to 'this'.",
          matches="this = @z",
          substitutes="<reject>")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
        Map<String, ParseTreeNode> bindings = match(node);
    ...
    new Rule() {
      ...
      @RuleDescription(
          name="setInitialize",
          synopsis="Expand the right side.",
          reason="",
          matches="var @v = @r",
          substitutes="var @v = @r")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
    ...
    new Rule() {
      ...
      @RuleDescription(
          name="setVar",
          synopsis="If LHS is a reference, expand the RHS.",
          reason="",
          matches="@v = @r",
          substitutes="@v = @r")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
    ...
    new Rule() {
      ...
      @RuleDescription(
          ...
          reason="",
          matches="new @ctor(@as*)",
          substitutes="___.construct(___.asSimpleFunc(@ctor), [@as*])")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
        Map<String, ParseTreeNode> bindings = match(node);
        if (bindings != null) {
          return subst(bindings);
