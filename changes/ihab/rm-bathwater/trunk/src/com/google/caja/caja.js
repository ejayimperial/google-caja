...
//    fastpathSet(obj, name);
  ...
  function isTamedFunc(func) {
    return (typeof func === 'function') && !!func.TAMED___;
  }

  /**
   * Marks an anonymous function as tamed:
   * the function is externally provided but
   * is safe to call.
   * <p>
   * @param opt_name if provided, should be the message name associated
   *   with the function. Currently, this is used only to generate
   *   friendlier error messages.
   */
  function tamedFunc(fun, opt_name) {
    enforceType(fun, 'function', opt_name);
    if (isSimpleFunc(fun)) {
      fail("Internal: Simple functions can't be tamed: ", fun);
    }
    fun.TAMED___ = true;
    if (opt_name) {
  ...
  /** Only tamed functions can be called as tamed functions */
  function asTamedFunc(fun) {
    if (isTamedFunc(fun) || isSimpleFunc(fun)) {
      return fun;
    }
    enforceType(fun, 'function');
    fail("Untamed functions can't be called as tamed functions: ", fun);
  }
  ...
  function asFirstClass(value) {
    switch(typeof value) {
      case 'function': {
        if (((isSimpleFunc(value) || isTamedFunc(value)) &&
             isFrozen(value)) ||
  ...
  function canCallPub(obj, name) {
    ...
    if (name === 'toString') { return false; }
    var func = obj[name];
    if (!isSimpleFunc(func) && !isTamedFunc(func)) {
      return false;
    }
    fastpathCall(obj, name);
    return true;
  ...
  function grantToString(proto) {
    proto.TOSTRING___ = tamedFunc(proto.toString, 'toString');
  }
...
//    this.TOSTRING___ = xo4a(this.toString, 'toString');
  ...
  handleGeneric(Function.prototype, 'apply', function(self, realArgs) {
    return asTamedFunc(this).apply(self, realArgs);
  });
  handleGeneric(Function.prototype, 'call', function(self, var_args) {
    return asTamedFunc(this).apply(self, Array.slice(arguments, 1));
  });
  ...
  simpleFunc(EvalError);
  simpleFunc(RangeError);
  simpleFunc(ReferenceError);
  simpleFunc(SyntaxError);
  simpleFunc(TypeError);
  simpleFunc(URIError);
  ...
  /**
   * <tt>caja.construct(ctor, [args...])</tt> invokes a simple function as
   * a constructor using 'new'.
   */
  function construct(ctor, args) {
    ctor = asSimpleFunc(ctor);
