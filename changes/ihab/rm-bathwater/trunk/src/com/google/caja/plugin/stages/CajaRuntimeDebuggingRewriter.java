...
final class CajaRuntimeDebuggingRewriter extends Rewriter {
...

  {
    ...
    addRule(new AddPositionParamRule() {
      ...
      @RuleDescription(
          name="asCtor",
          synopsis="adds debug info to caja.construct calls",
          reason="",
          matches="caja.construct(@fun, @args)",
          substitutes="caja.construct(@fun, @args, @debug)")
      public ParseTreeNode fire(ParseTreeNode n, Scope s, MessageQueue mq) {
        return super.fire(n, s, mq);
      }
    });
    ...
    // TODO(ihab.awad): Substitute with another rule for caja.each().
    /*
    addRule(new AddPositionParamRule() {
          @Override
          @RuleDescription(
