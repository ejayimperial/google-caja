...
(function () {
  ...
  function construct(fun, args, callerIdx) {
    if (!fun.isDebugWrapper___) {
      fail('Function leaked through debug wrapper');
    }
    fun = fun.originalFunction___;
    if (!fun.SIMPLEFUNC___) {
      fail('Non simple function was unwrapped');
    }
  ...
  function makeWrapper(fun, conditionName, stackFrame) {
    ...
    wrapper.isDebugWrapper___ = true;
    wrapper.originalFunction___ = fun;
