...
ContentLine = function(opt_name, opt_value, opt_attributes) {
   ...
   * @type {Array.<string>}
   */
  var attributes_ = opt_attributes ? opt_attributes.slice() : [];
  /**
  ...
  return {
    ...
    toString: function () {
      var out = [name_];
      for (var i = 0, n = attributes_.length; i < n; i += 2) {
        var paramValue = attributes_[i + 1];
        if (/[:;]/.test(paramValue)) {
          paramValue = '"' + paramValue + '"';
        }
        out.push(';', attributes_[i], '=', paramValue);
      }
      out.push(':');
      for (var i = 0, n = values_.length; i < n; ++i) {
        if (i) { out.push(','); }
        if (!noEscape_) {
          out.push(values_[i].replace(/([;,\\])/g, '\\$1')
                   .replace(/\r\n?|\n/g, '\\n'));
        } else {
          out.push(values_[i]);
        }
    ...
    pushAttributes: function (var_args) {
      attributes_.push.apply(attributes_, arguments);
    },

    setAttributes: function (attributes) {
      attributes_ = attributes.slice(0);
    },
     ...
     * @return {Array.<string>}
     */
    getValues: function () { return values_; },

    pushValues: function (var_args) {
      values_.push.apply(values_, arguments);
    },
