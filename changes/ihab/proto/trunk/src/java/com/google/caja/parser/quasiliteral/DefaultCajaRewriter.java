...
public class DefaultCajaRewriter extends Rewriter {
  final public Rule[] cajaRules = {
    ...
    new Rule () {
      @Override
      @RuleDescription(
          name="readGlobalProto",
          synopsis="Allow reading of the ___proto___ property of a global object",
          reason="This field is used extensively by prototype.js")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
        Map<String, ParseTreeNode> bindings = new LinkedHashMap<String, ParseTreeNode>();
        if (match("@x.__proto__", node, bindings)
            && scope.isGlobal()
            && bindings.get("x") instanceof Reference) {
          return substV(
              "___.readPub(___OUTERS___.@x, '__proto__');",
              "x", bindings.get("x"));
        }
        return NONE;
      }
    ...
    new Rule () {
      ...
      @RuleDescription(
          name="readProto",
          synopsis="Allow reading of the ___proto___ property of an object",
          reason="This field is used extensively by prototype.js")
      public ParseTreeNode fire(ParseTreeNode node, Scope scope, MessageQueue mq) {
        Map<String, ParseTreeNode> bindings = new LinkedHashMap<String, ParseTreeNode>();
        if (match("@x.__proto__", node, bindings)) {
          return substV(
              "___.readPub(@x, '__proto__');",
              "x", bindings.get("x"));
        }
        return NONE;
      }
