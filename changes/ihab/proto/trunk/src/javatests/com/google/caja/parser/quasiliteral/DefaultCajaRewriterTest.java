...
public class DefaultCajaRewriterTest extends CajaTestCase {
  ...
  /**
   * The "Rhino proto hack" is a hack that works around weird behavior. In Rhino, for some
   * object 'x', the direct property accessor expression --
   *   x.__proto__
   * returns different results from the indexed accessor expression --
   *   x['__proto__']
   * Since Caja translates direct to indexed property accessors (via readPub), this causes
   * our tests to fail spuriously.
   */  
  private boolean rhinoProtoHackEnabled = false;
  ...
  public void testReadGlobalProto() throws Exception {
    rhinoProtoHackEnabled = true;
    assertConsistent(
        "  var x = { };"
        + "(x.__proto__ - 0);");
    assertConsistent(
  ...
  private void assertConsistent(String message, String caja)
      throws IOException, ParseException {
    Object uncajoledResult = runUncajoled(caja);
    Object cajoledResult = runCajoled(caja);
    System.err.println("Result: " + cajoledResult + " for " + getName());
    assertEquals(message, uncajoledResult, cajoledResult);
  }

  private Object runUncajoled(String caja)
      throws IOException, ParseException {
  ...
  private Object runCajoled(String caja) 
      throws IOException, ParseException {
    ...
    return RhinoTestBed.runJs(
        ...
        new RhinoTestBed.Input(
            rhinoProtoHackEnabled ?
              "  (function() {"
              + "  var originalReadPub = ___.readPub;"
              + "  ___.readPub = function(obj, name, opt_shouldThrow) {"
              + "    if ('__proto__' == name) {"
              + "      if (___.canReadPub(obj, name)) {"
              + "        return obj.__proto__;"
              + "      } else {"
              + "        throw 'Cannot read __proto__';"
              + "      }"
              + "    } else {"
              + "      return originalReadPub(obj, name, opt_shouldThrow);"
              + "    }"
              + "  }"
              + "})();" :
              ";",
            getName() + "-rhino-proto-hack"),
